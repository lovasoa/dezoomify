...
function fakeElement() {
  ...
  return {
    ...
    getImageData() { throw new Error("preview must not read pixels"); },
    toBlob() { throw new Error("preview must not encode"); },
    toDataURL() { throw new Error("preview must not encode"); },
  };
}
...
test("website preview wires wheel, drag, and buttons without pixel reads", () => {
  const doc = fakeDoc();
  for (const id of ["canvas-wrapper", "rendering-canvas", "preview-zoom-in", "preview-zoom-out", "preview-zoom-fit", "preview-zoom-100", "preview-zoom-label"]) {
    doc.element(id);
  }
