...
test("coordinator logs active-tab and job-bus interactions", async () => {
    ...
    return [
      {
        ...
        result: {
          ok: true,
          documentUrl: "https://gallery.example/work",
          inputs: [{ url: "https://gallery.example/tiles/a.jpg" }],
          overflow: 0,
        },
      },
    ];
  ...
  assert.match(
    entries.find((entry) => entry.code === "active-tab-op-result").detail,
    /candidates=1/,
  );
});
