...
fn test_headless_browser() -> Result<(), String> {
    // Fresh checkouts have no `wasm/` glue (gitignored, generated by
    // `cargo xtask build web`); the headless tests stage real store
    // packages via package-store.sh, which requires the glue. Ensure it
    // here too, not just in `build_extension`. Fast path when present.
    ensure_wasm_glue()?;
