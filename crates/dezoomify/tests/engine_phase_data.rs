//! Phase-grouped state proofs for `crates/dezoomify/src/engine/job.rs`.
//!
...
fn stale_generation_is_rejected_and_pending_survives() {
    ...
    assert_eq!(job.snapshot().lifecycle, JobState::AwaitingPartialDecision);
    job.command(dezoomify::engine::UserCommand::AnswerPartial {
        generation,
        decision: RecoveryChoice::Keep,
    })
...
fn selection_locks_across_phase_exits() {
    ...
    // awaiting the image.
    let err = job
        .command(dezoomify::engine::UserCommand::SelectLevel { level: 0 })
        .unwrap_err();
    assert_eq!(err.code, "job.invalid-state");
    assert!(job.snapshot().selection.level.is_none());

    job.command(dezoomify::engine::UserCommand::SelectImage { image: 0 })
        .expect("select image");
    // The decided image cannot change: a different image is invalid-state.
    let revision = job.snapshot().revision;
    let err = job
        .command(dezoomify::engine::UserCommand::SelectImage { image: 1 })
        .unwrap_err();
    assert_eq!(err.code, "job.invalid-state");
    assert_eq!(job.snapshot().revision, revision);
    ...
    let err = job
        .command(dezoomify::engine::UserCommand::SelectLevel { level: 99 })
        .unwrap_err();
    assert_eq!(err.code, "job.invalid-state");
}
...
fn probe_and_tile_answers_never_cross() {
    ...
    let kinds: Vec<bool> = update
        .tile_effects()
        .iter()
        .map(|e| matches!(e, Effect::AcquireTile { request, .. } if request.purpose == dezoomify::model::RequestPurpose::Probe))
        .collect();
    assert!(
