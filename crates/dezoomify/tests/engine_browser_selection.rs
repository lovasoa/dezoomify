...
fn browser_policy_follows_deferred_catalog_and_selects_a_fitting_level_in_place() {
    ...
    let initial_id = update.metadata_effects()[0]
        .correlation()
    ...
    let update = job
        .provide_metadata(
            follow[0].correlation().expect("correlated effect"),
    ...
    let dezoomify::model::HostEffect::AcquireTile {
        request, placement, ..
    } = tiles[0]
    else {
    ...
    assert!(
        request.uri.contains("_files/7/"),
        "selected DZI path: {}",
        request.uri
    );
    assert_eq!(placement.canvas.as_ref().map(|size| size.width), Some(128));
}
