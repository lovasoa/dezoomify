...
use dezoomify::engine::{
    DiscoveryInput, EffectId, EffectResult, EngineJob, Failure, JobOptions, PartialPolicy,
    ResponseMetadata, SelectionPolicy,
};
...
fn metadata_id(update: &dezoomify::engine::Update) -> EffectId {
    let effects = update.metadata_effects();
    assert_eq!(effects.len(), 1, "one metadata effect: {effects:?}");
    effects[0].correlation().expect("correlated effect")
}

fn tile_ids(update: &dezoomify::engine::Update) -> Vec<EffectId> {
    update
        .tile_effects()
        .iter()
        .map(|e| e.correlation().expect("correlated effect"))
        .collect()
}

fn tile_ordinals(update: &dezoomify::engine::Update) -> Vec<u32> {
    update
        .tile_effects()
        .iter()
        .map(|e| match e {
...
fn start_dzi() -> (EngineJob, dezoomify::engine::Update) {
...
fn select_largest(job: &mut EngineJob) -> dezoomify::engine::Update {
    let update = job
        .command(dezoomify::engine::UserCommand::SelectImage { image: 0 })
        .expect("select image");
    assert_eq!(update.snapshot.lifecycle, JobState::AwaitingLevelSelection);
    let level = update.snapshot.selection.level_count - 1;
    let update = job
        .command(dezoomify::engine::UserCommand::SelectLevel { level })
        .expect("select level");
...
fn deferred_cycle_and_budget_are_rejected_without_new_work() {
    ...
    let err = job
        .command(dezoomify::engine::UserCommand::FollowDeferred { image: 0 })
        .unwrap_err();
...
fn transient_retry_budget_has_exact_effect_counts_and_delays() {
    ...
    let (timer, attempt, delay) = match &update.effects[0] {
        Effect::WaitRetryTimer {
            ...
            delay_ms,
            ..
        } => (EffectId(*effect), *attempt, *delay_ms),
    ...
    let retry_id = match &update.effects[0] {
        Effect::AcquireTile { request, .. } => EffectId(request.id),
...
fn partial_retry_requeues_exactly_failed_tiles_preserving_good() {
    ...
    let update = job
        .command(dezoomify::engine::UserCommand::AnswerPartial {
            generation,
...
fn cancel_and_publication_ordering_is_exact() {
    ...
    job.complete(ids[0], EffectResult::TileAcquired)
        .expect("first tile");
    let update = job
        .command(dezoomify::engine::UserCommand::Cancel)
        .expect("cancel");
    assert_eq!(update.snapshot.lifecycle, JobState::Cancelled);
    assert_eq!(update.effects.len(), 1);
    assert!(matches!(update.effects[0], Effect::CancelWork));
    assert!(matches!(
...
fn pause_defers_retry_timers_until_resume() {
    ...
    let update = select_largest(&mut job);
    let ids = tile_ids(&update);
    job.command(dezoomify::engine::UserCommand::Pause)
        .expect("pause");
    ...
    let update = job
        .command(dezoomify::engine::UserCommand::Resume)
        .expect("resume");
