use dezoomify_engine::{
    DiscoveryInput, Effect, EngineJob, JobOptions, ResponseMetadata, SelectionPolicy,
};
...
fn native_image_index_clamps_and_follows_deferred_entry_in_same_job() {
    ...
    let follow_uri = deferred
        ...
        .find_map(|effect| match effect {
            Effect::AcquireMetadata { uri, .. } => Some(uri.as_str()),
            _ => None,
        })
    ...
    let follow = deferred.metadata_effects()[0].id();
    let selected = job
...
fn native_deferred_follow_rejection_keeps_typed_failure() {
    ...
    let Some(SnapshotTerminalDto::Failed { error }) = update.snapshot.terminal else {
