...
test("fetchMetadataFor serves direct bytes without the proxy", async () => {
  ...
  assert.deepEqual(
    attempts.map(({ transport, outcome, bytes }) => ({ transport, outcome, bytes })),
  ...
  const fetcher = createWebFetcher({
    ...
    proxyTransport: {
      fetchViaProxy: async () => ({
        ...
        finalUrl: "https://a.test/upstream.json",
      }),
    },
    isProxyEligible: () => ({ eligible: true, reason: "public" }),
    hooks: h,
...
test("fetchMetadataFor never proxies HTTP errors or ineligible targets", async () => {
  ...
  await assert.rejects(
    () => fetcher.fetchMetadataFor("https://a.test/x", {}),
    (e) => {
  ...
  await assert.rejects(
    () => direct429.fetchMetadataFor("https://a.test/x", {}),
    (e) => {
...
test("metadata failures carry their typed cause plus the structured details context", async () => {
  ...
  await assert.rejects(
    () => direct403.fetchMetadataFor("https://a.test/x?query=1", {}),
    (e) => {
      assert.equal(e.code, "DISCOVERY_HTTP_ERROR");
  ...
  await assert.rejects(
    () => policy.fetchMetadataFor("https://a.test/x?sig=1", {}),
    (e) => {
  ...
  const proxy404 = createWebFetcher({
    ...
    proxyTransport: {
      fetchViaProxy: async () => ({ ok: false, status: 404, code: "TRANSPORT_HTTP_ERROR" }),
    },
    isProxyEligible: () => ({ eligible: true, reason: "public" }),
    hooks: h,
    messages,
  ...
  await assert.rejects(
    () => proxy404.fetchMetadataFor("https://a.test/missing", {}),
    (e) => {
  ...
  await assert.rejects(
    () => network.fetchMetadataFor("https://a.test/x", {}),
    (e) => {
...
test("fetchMetadataFor retries a transient proxy throttle once", async () => {
  ...
  const fetcher = createWebFetcher({
    ...
    proxyTransport: {
      fetchViaProxy: async () => {
        calls += 1;
        if (calls === 1)
          return { ok: false, status: 429, code: "PROXY_RATE_LIMITED", retryAfterMs: 10 };
  ...
  const persistent = createWebFetcher({
    ...
    proxyTransport: {
      fetchViaProxy: async () => ({ ok: false, status: 429, code: "PROXY_RATE_LIMITED" }),
    },
    isProxyEligible: () => ({ eligible: true, reason: "public" }),
    hooks: h,
    messages,
  ...
  await assert.rejects(
    () => persistent.fetchMetadataFor("https://a.test/x.json", {}),
    (e) => {
      assert.equal(e.code, "UPSTREAM_RATE_LIMITED");
...
test("fetchTileFor performs one attempt and returns retryable failures to the engine", async () => {
  ...
  await assert.rejects(
    () => fetcher.fetchTileFor("https://a.test/1.png", {}),
    (e) => {
  ...
  await assert.rejects(
    () => failing.fetchTileFor("https://a.test/1.png", {}),
    (e) => {
      assert.equal(e.code, "TILE_FAILED");
      return true;
    },
