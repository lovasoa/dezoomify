...
export function createTilePainter(deps: TileDrawDeps): TilePainter {
  ...
  async function drawTile(
    ...
    const drawBitmap = async (source: TileBitmap | TileImageLike): Promise<void> => {
      ...
      const fullW = isImage
        ? (source as TileImageLike).naturalWidth
        : (source as TileBitmap).width;
      const fullH = isImage
        ? (source as TileImageLike).naturalHeight
        : (source as TileBitmap).height;
      // Trust the plan for placement: the canvas layout must stay seamless
      // even when a tile decodes at an unexpected size. Log the mismatch and
      // scale the decoded bytes to the planned extent so no gap appears.
