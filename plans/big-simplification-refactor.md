...
The implementation is complete and the architecture now follows the ownership rules above:

- The Rust engine owns automatic browser and native image/level selection, deferred following, retries, partial decisions, and the authoritative job snapshot. Selected image and level positions are stored once.
- Browser and native hosts execute effects and own resources only. Native execution no longer keeps a second catalog or selection policy.
- The shared UI renders snapshots. It contains no dormant image/level picker, callbacks, presentation model, styles, or localized picker copy. Generated catalogs and typed selection commands remain available for a future product integration.
- The website owns its URL queue payload and validation wrapper; `app-model` owns generic FIFO behavior and the canonical input URL validator.
- Browser transport state uses stable protocol codes. `app-model` owns their display labels; the shared UI has no compatibility re-export facade or legacy aliases.
- Browser output finalization reports the disposition the product actually performed: ready for the website, initiated for the extension, and display-only for tainted output.
- Extension tab bindings live in memory. The extension adds no storage permission and requires a fresh toolbar start after a background-worker restart.
- The legacy website remains unchanged at `/`; the Vite application remains at `/beta`. Website generated output is still excluded from version control.
- Removed implementations include unreachable native-session handoff code, unused browser cache claims/helpers, stale test-only APIs, duplicate queue tests, and obsolete manual-selection UI code.

`cargo xtask ci local` passes on the completed tree. The host currently runs Node 22 and prints the expected Node 24 engine warning; TypeScript, Rust, fixture, architecture, protocol, browser, desktop, extension, end-to-end, and supply-chain gates all complete successfully.

The diagnosis and original four-agent brief below are retained as historical design context. Their old paths and proposed work are not current implementation instructions.
