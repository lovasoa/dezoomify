...
workers, browser image decoding, display and save canvases, resource limits, and a
bounded optional browser cache for non-secret data. The runtime must support two
deliberately different tile treatments:

1. **Ordinary image display:** load cross-origin tiles as ordinary `<img>`
   ...
   guidance where the browser supports it. The canvas records
   `originClean = false` and forbids only programmatic pixel reads, processing,
   hashing, `toBlob`, `toDataURL`, and claims of clean save.
2. **Readable-byte fetching:** obtain readable metadata and tile bytes with `fetch` or
   an injected browser-session transport, decode and process them, composite only
   origin-clean pixels, and save a clean file. Website proxy policy is owned by
   phase 09; the extension browser-session transport is owned by phase 12.

The browser runtime owns discovery-effect execution, transport execution,
cancellation, bounded scheduling, image decoding, compositing, save preflight,
and structured progress. Hosts supply app-specific transport
eligibility and fallback policy; the runtime does not own app-specific UI.
...
- Do not treat canvas taint as a display failure. Do not attempt programmatic
  readback, processing, hashing, `toBlob`, `toDataURL`, or clean save when
  `originClean` is false.
- Do not add a general-purpose proxy, select proxy capability before a
  classified CORS/network failure, or conceal the active transport. Phase 09 may
  automatically select its metadata CORS proxy capability after that failure when
  the metadata request is eligible and the user has not opted out; tiles never
  use the proxy.
- Do not read cookies, accept cookie strings, or add browser cookie APIs.
- Do not duplicate format or tile-grid logic in TypeScript; that remains in the Rust core/Wasm boundary.
...
- `packages/browser-runtime/src/save.ts`
...
- Added during step 17: `cargo xtask test browser`, including its `--build-only`
  mode.
- Until those xtask commands exist, use the direct commands shown in the relevant validation blocks. Do not report a missing future command as a failure.

## Sequential Implementation Steps

1. Verify the phase boundary. Run `git status --short`, `cargo xtask protocol check`, `cargo xtask test`, `cargo test --workspace`, and the root pnpm typecheck command established in phase 05. Record pre-existing failures in `artifacts/phase-08/preflight.md`; do not edit files responsible for unrelated failures. Confirm the phase-07 WASM package exposes only its own capabilities and does not claim browser transport, worker, canvas, cache, decode, or native-only encoder support; phase 08 adds the browser-runtime capability report.
...
2. Add `packages/browser-runtime/package.json` and `tsconfig.json`. Consume the
   generated `@dezoomify/wasm` package, whose stable entrypoint is
   `target/wasm-package/index.js`, through the exact local dependency
   `file:../../target/wasm-package`; phase-07's build stages that package before
   installation and the pnpm lock records the local dependency. Also depend on
   `packages/protocol-ts`. Do not add an unresolved registry dependency or
   require generated output to be checked in. Keep React, Vite, Playwright, and
   app code out of runtime dependencies. Export the public entry point and
   make tests use the repository-standard runner.

   **Validate immediately:** run `cargo xtask build wasm` to stage the local
   package, then run `pnpm install --frozen-lockfile` after the workspace entry is
   present; if registration changes the lockfile, use the repository-approved
   pnpm lockfile update command first and inspect only expected workspace
   metadata. Then run `pnpm --filter ./packages/browser-runtime typecheck`.

3. Define public runtime types in `src/types.ts`: `BrowserSession`, `BrowserSessionEvent`, `TileTransport`, `TileResponse`, `TileSurface`, `SaveCapability`, `BrowserLimits`, and explicit error codes. Model transport outcomes as `readable`, `ordinary-image-allowed`, `http-error`, `network-error`, `cancelled`, and `policy-denied`. Every surface reports `originClean`; never represent an ordinary image load as readable bytes.

   **Validate immediately:** add compile-time exhaustive-switch tests and run `pnpm --filter ./packages/browser-runtime test -- --run types`. Verify that an object containing both `ordinaryImage: true` and `bytes` does not type-check and that `originClean: false` removes all programmatic save/read/process capabilities.

4. Implement `src/transport.ts` with a direct browser transport. Use `fetch`
...
8. Implement `src/save.ts`. Permit programmatic save only from an origin-clean readable-byte surface; use `canvas.toBlob` or the repository-owned deterministic encoder; include decoded output dimensions, actual MIME type, and a suggested safe filename; reject null blobs and unsupported formats with protocol error codes. For an `originClean = false` display surface return `SAVE_REQUIRES_READABLE_BYTES` before invoking any browser save API, while preserving browser/user-agent save guidance.

   **Validate immediately:** save the deterministic 2x2 readable scenario and compare decoded output pixels, dimensions, and MIME in all three engines. Compare exact encoded SHA-256 only when using the deterministic repository-owned encoder. For the tainted display scenario, assert the capability/UI never calls the JavaScript save entry point and instrumentation records no pixel read, processing, hash, `toBlob`, or `toDataURL`; the controlled readback test, not app code, owns the expected `SecurityError`. Run `pnpm --filter ./packages/browser-runtime test -- --run save`.

9. Implement `src/worker.ts`, `worker-client.ts`, `buffers.ts`, and `session.ts` as the only coordinator boundary. The browser runtime, not the shared UI or an app integration, owns worker creation/lifecycle, each Wasm session, image decode dispatch, and surface coordination. The main-thread client executes browser-only effects, transfers readable `ArrayBuffer` ownership rather than cloning/base64 encoding, applies byte-count backpressure, and returns correlated results. Choose the canvas only after the caller approves ordinary image display or readable bytes, preserve deterministic commit order, and emit monotonic sequence-numbered events. Make cancellation/disposal idempotent, terminate failed/replaced workers, release every Wasm/JS buffer handle, and ensure late responses cannot mutate a cancelled or replaced session.
...
10. Add a fallback-policy API. Only after classifying a direct browser attempt
    as a CORS/network failure, return structured allowed next transports supplied
    by the host. The runtime must not know metadata CORS proxy URLs,
    extension IDs, native protocol URLs, host installation state, or proxy
    eligibility policy. It may identify ordinary image display, the metadata CORS
    proxy, the extension, or native only through protocol capability identifiers
    and must emit the active transport on every attempt. The phase-09 web
    integration may automatically select the metadata CORS proxy when its public
    non-credential metadata-target policy allows it and proxy fallback is not
    opted out; tiles never use the proxy; automatic fallback
    requires no additional user decision.
...
13. Add Playwright tests in Chromium, Firefox, and WebKit for both paths. For ordinary image display, assert visible assembled layout, no `crossorigin`, no response-body reads, `originClean = false` after display-canvas draw, displayed save guidance, a controlled readback `SecurityError`, and zero app attempts to read/process/hash/`toBlob`/`toDataURL`/clean-save. For readable bytes, assert direct CORS fetch, origin-clean readback, decoded pixels/dimensions/MIME, cancellation, and cleanup. Require exact encoded hashes only for the repository-owned deterministic encoder. Mark no browser-specific expected failures without a linked issue and expiry date.
...
14. Add browser-runtime build integration in `crates/xtask/src/browser.rs`.
    The internal helper must run the phase-07 Wasm build, stage the generated
    `@dezoomify/wasm` package at `target/wasm-package`, verify generated
    bindings are clean, and run the browser-runtime TypeScript build without
    executing browser tests. Use argument arrays rather than shell
    interpolation. The public build-only mode is owned by step 17.

    **Validate immediately:** run `cargo xtask build wasm` to stage the generated
    package, then run `pnpm --filter ./packages/browser-runtime build` twice. The
    second run must leave `git status --short` unchanged and must not rewrite
    generated files nondeterministically.

15. Add bundle and dependency boundary checks. Fail if Node built-ins, filesystem APIs, native runtime crates, Tauri APIs, extension APIs, or shared UI packages enter the browser-runtime graph. Set an explicit compressed bundle budget based on the phase-07 baseline plus the reviewed runtime allowance.

    **Validate immediately:** run `pnpm --filter ./packages/browser-runtime lint`, the repository dependency-boundary test, and the direct Wasm/package and browser-runtime build commands. Inspect the emitted dependency report in `artifacts/phase-08/`.

16. Add security regression tests. Feed hostile filenames, SVG tiles, malformed image bytes, oversized dimensions, redirect loops, credential-bearing URLs, HTML error pages, and secret-bearing responses offered to cache. Ensure no HTML is injected, no URL credentials are logged or cached, object URLs are revoked, tainted surfaces cannot reach programmatic pixel/save APIs, and errors are bounded in size.
...
17. Complete `cargo xtask test browser` so it starts the deterministic server on
    allocated loopback ports, stages/builds Wasm and the harness, runs all three
    Playwright projects, stops child processes on success or failure, and writes
    machine-readable results to `artifacts/phase-08/results.json`. Its
    `--build-only` mode performs the same staging and TypeScript build without
    browser tests.

    **Validate immediately:** run `cargo xtask test browser --build-only`, then
    run `cargo xtask test browser` twice, once normally and once with
    `DEZOOMIFY_TEST_CONCURRENCY=1`. Verify no server process or temporary profile
    remains. The browser target and its build-only mode become available only
    after this step.

18. Run the final phase gate: `cargo fmt --all -- --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace`, the root pnpm lint/typecheck commands, `cargo xtask protocol check`, `cargo xtask test`, `cargo xtask test browser --build-only`, and `cargo xtask test browser`.
...
7. Verify programmatic save is disabled with `Readable tile bytes are required` and browser/user-agent save guidance is visible where supported.
...
4. Verify app save is unavailable and the capability reason is `SAVE_REQUIRES_READABLE_BYTES`; do not invoke the JavaScript save entry point.
5. Verify instrumentation reports no app save call, pixel read, process, hash, `toBlob`, or `toDataURL` attempt and that save guidance remains visible.
...
- Stop if ordinary image display cannot remain visible after taint, or if app
  code attempts a pixel read, processing, hash, `toBlob`, `toDataURL`, or clean
  save while `originClean` is false.
...
- **Canvas taint is misclassified:** track `originClean` explicitly, prove taint
  with a controlled `SecurityError`, and instrument app code to forbid only
  programmatic pixel/save operations while retaining visible assembly and save
  guidance.
...
- **Unbounded browser memory:** preflight limits, cap concurrency, close decoded resources, and hand off large images to native.
...
- [ ] Controlled readback proves `SecurityError`, app code makes no programmatic read/process/hash/save attempt, and save guidance is displayed.
- [ ] Readable tile bytes produce an origin-clean save with expected decoded
...
- [ ] CORS/network-denied direct browser fetch is typed correctly; fallback capabilities
...
- [ ] `cargo xtask test browser --build-only` passes twice without diff.
