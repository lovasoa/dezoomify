...
`crates/dezoomify/src/model.rs` is the single source of truth for values crossing the Rust/TypeScript line. Its Rust definitions derive `Serialize`, `Deserialize`, and `Tsify`. The real `wasm-bindgen` declaration is tracked in `packages/wasm-bindings` and imported by every TypeScript boundary.
...
sequenceDiagram
    ...
    S-->>JS: DispatchResult with ordered HostEffect[] + Snapshot
    JS->>S: complete(HostCompletion)
    S-->>JS: DispatchResult with ordered HostEffect[] + Snapshot
    JS->>S: snapshot()
    S-->>JS: current Snapshot
    JS->>S: dispose()
...
- a successful result contains ordered `HostEffect[]` values plus the
  absolute `Snapshot` after the answer;
- `snapshot()` returns the current `Snapshot` without dispatching;
...
`HostEffect` (resource/tile/probe acquisition, finalization, cancellation, recovery decisions) is exhaustive; boundaries handle it through typed tables. Job state crosses as absolute `Snapshot` projections, never as event walks. Effect meanings are in the [host-effect contract](job-engine.md#host-effect-contract). The retry timer effect is `wait-retry-timer{effect, tile, attempt, delay_ms}`: the host waits `delay_ms` on its own clock and then answers with the matching `RetryTimerElapsed{effect}`. The engine parks ready retries while paused; the host only waits and reports completion. Tile failures cross as `TileFailure{code, category, http, retry_after_ms, detail}` with bounded diagnostics; `FetchFailure` carries the host-observed `retry_after_ms` when the response carries one. The engine backoff is a 1 s base doubling to a 30 s ceiling, with an observed `retry-after` honored to 300 s. Tile acquisition carries `TilePlacement{position, expected_size, canvas, processing, probe_output}` so hosts assemble without re-deriving geometry. The format grid includes `second_canvas`.
