...
dezoomify-ng treats source websites, image metadata, tiles, handoff envelopes, and output names as untrusted input. Each runtime grants only the access needed for the active user-requested job.

## Trust boundaries

- Web Studio runs under normal browser origin rules.
- The Cloudflare proxy is a restricted public, non-credential resource fetcher, not a trusted credential endpoint.
...
Web Studio attempts direct readable fetch first. Only a classified CORS or network failure can trigger automatic Cloudflare proxy fallback, and only when proxy use is enabled and the resource is an eligible public, non-credential `http` or `https` destination. There is no per-attempt proxy consent. Studio shows the active transport and honors opt-out by issuing no new proxy requests while the preference is disabled.

The proxy permits only supported HTTP methods. It resolves and rejects loopback, private, link-local, reserved, and cloud-metadata addresses before connecting and after redirects. It rechecks public-resource and credential eligibility across redirects; bounds redirects, response bytes, duration, and concurrency; validates expected metadata or image content; omits credentials; strips request and response headers outside an allowlist; and applies abuse controls without recording sensitive URLs. Authentication failures and ordinary HTTP application errors do not qualify as CORS or network failures and do not activate the fallback.

## Extension and desktop

Extension scans begin only with an explicit action, register before at most one reload, stop at a finite deadline, and never reload-rearm; see [Extension](extension.md). Content scripts cannot invoke arbitrary privileged fetches. Tauri exposes an allowlisted command surface and passes opaque file handles instead of unrestricted paths where practical.
