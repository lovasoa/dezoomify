...
One host-neutral view renderer (vanilla TypeScript, no UI framework, no
bundler) renders discovery, selection, job progress, recovery, and output.
Thin integrations connect it to the web worker, Tauri commands, or extension
messaging. The TypeScript sources are the single source of truth; the
browser-served `.js` mirrors are generated by `scripts/sync-web-js.mjs`
(never hand-edited). The shared UI depends on generated TypeScript protocol
types, not host-specific implementation details.
