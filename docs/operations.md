...
Steps 1 and 2 also run in CI: dispatch `release-build` with the tag, then `release-sign` and `release-publish` with the run ids; in CI the signing key comes from the `release-signing` environment secret, and every stage fails closed when the key, digests, or signatures are missing. The extension artifact from step 2 is the store payload for the existing listings (Chromium `iapjjopjejpelnfdonefbffahmcndfbm`, Firefox AMO guid `{14074c89-8a5f-4813-98df-a7117f062871}`), submitted through the `store-submit` workflow; never create a new store item. Chromium uploads are drafts until `publish=true`; a failed Chromium publish can be retried with `publish=true` and `upload=false` without replacing the accepted draft. AMO updates upload and validate the package first, then submit its upload UUID to the existing add-on on the listed channel, which enters review immediately; listing metadata (description, links, release notes) is dashboard-only and never set by the API.
