...
Extension tests pin narrow grants, no remote code, required CSP, explicit-action scans, finite capped operations, session fetch scope, and lifecycle cleanup. Packaged Chromium and Firefox tests exercise `scan()` and authenticated `fetch()` directly from the job page, verify navigation invalidation and the saved PNG, and cover the optional-host and partial flows. Chromium also restarts the background worker and verifies the job page still reaches the source tab. Tainted-display tests keep `<img>` display visible while guarding pixel reads, hashing, `toBlob`, and `toDataURL` without readable bytes. Contract: [Extension](extension.md).
